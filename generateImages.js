...
const priorities = ['punks','top','cheek', 'glasses'];

module.exports.generateNFT = async (numberOfOutputs) => {
...
module.exports.recreateOutputsDir = () => {
  ...
  fs.mkdirSync(`${dir.outputs}/export`);
};
...
const drawImage= async (traitTypes, background, index) => {
  const paddedHex = ("" + index.toString(16)).substr("-64");
  // draw background
  ...
  // save metadata
  fs.writeFileSync(
    `${dir.outputs}/metadata/${paddedHex}.json`,
    JSON.stringify({
      name: `punk ${index}`,
  ...
  // save image 
  fs.writeFileSync(
    `${dir.outputs}/export/${paddedHex}.png`, 
