...
const createDna = (_races, _race) => {
  ...
  _races[_race].layers.forEach((layer) => {
    ...
    layer.elements.forEach((element) => {
      ...
      if (randElementNum < totalWeight) { // 100 must match the # on line 120
        num = element.id;
        existNum = true;
      }
    });
...
const writeMetaData = (_data) => {
  // fs.writeFileSync("../outputs/_metadata.json", _data);
};

const saveMetaDataSingleFile = (_editionCount) => {
  fs.writeFileSync(
    `./outputs/metadata/${_editionCount}`,
    JSON.stringify(metadataList.find((meta) => meta.edition == _editionCount))
  );
};

module.exports.startCreating = async () => {
  writeMetaData("");
